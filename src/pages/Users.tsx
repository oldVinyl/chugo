...
const StatusPanel: React.FC<{ activeUser?: User }> = ({ activeUser }) => {
  if (!activeUser)
    return (
      <p className="p-6 bg-white rounded-3xl w-full h-full">
        Select a User to see their details
      </p>
    );

  return (
    <div className="p-6 bg-white rounded-3xl w-full h-full overflow-auto no-scrollbar">
      ...
      <div className="space-y-2">
        ...
        <input
          type="password"
          value={activeUser.passkey.toString()}
          placeholder="Passkey"
          className="w-full p-3 rounded-lg border focus:outline-none"
        />
        ...
        <div className="flex space-x-6 pt-2">
          <label className="flex items-center text-sm font-medium cursor-pointer">
            <input
              type="radio"
              name="user-role"
              checked={activeUser.role === "staff"}
              className="hidden"
            />
            <span
              className="w-4 h-4 mr-2 border rounded-full flex items-center justify-center transition-all duration-200"
              style={{
                backgroundColor:
                  activeUser.role === "staff" ? "var(--acc)" : "white",
              }}
            >
          ...
          <label className="flex items-center text-sm font-medium cursor-pointer">
            ...
            <span
              ...
              style={{
                ...
                backgroundColor:
                  activeUser.role === "admin" ? "var(--acc)" : "white",
              }}
            >
        ...
        <button
          className="w-full py-3 mt-10 text-white font-semibold rounded-lg transition duration-200"
    ...
    <div className="h-full w-full">
      <div className="w-full h-full rounded-3xl">
        <div className="min-h-screen">
          <div className="flex flex-col h-[85vh] lg:flex-row gap-3">
            <div className="lg:w-2/3 space-y-2 p-4 rounded-3xl bg-white overflow-auto no-scrollbar">
